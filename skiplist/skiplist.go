...
func Newnode(level int, key int, value string) *SkipNode {
	node := new(SkipNode)
	node.Key = key
	node.Value = value
...
func (list *SkipList) Insert(key int, value string) bool {
	list.mutex.Lock()
...
// 跳表的查询
func (list *SkipList) Get(key int) *SkipNode {
	list.mutex.RLock()
...
// 获取相应的前驱节点
// 调整前驱节点的next指针
func (list *SkipList) Delete(key int) interface{} {
	list.mutex.Lock()
	defer list.mutex.Unlock()
